"""
Train SimulationBuckPHNN on PLECS data in physical coordinates.

L and C are fixed (known from design). The model learns effective resistances
R_L and R_C plus a state-dependent delta_G network that captures switching
losses, diode drops, dead-time, and other nonlinear effects missed by the
simple averaged model.

Usage:
    python src/train_simulation_buck.py
"""
...
from phnn import SimulationBuckPHNN
...
MODEL_PATH = ROOT / "models" / "sim_buck_phnn.pt"
LOSS_PLOT_PATH = ROOT / "tests" / "sim_buck_phnn_loss.png"
TRAJ_PLOT_PATH = ROOT / "tests" / "sim_buck_phnn_trajectories.png"

# Fixed — known from design
L_TRUE = 100e-6
C_TRUE = 100e-6

# Learned resistance initial guesses
INIT_R_L = 0.20
INIT_R_C = 0.05
...
# Trajectory quality thresholds — primary success metric
RMSE_I_TOL = 0.15   # A
RMSE_V_TOL = 0.20   # V
...
def stack_multistep_rollout(
    ...
    Returns:
        ...
        u:  [M, N, 2] input sequences [alpha * V_in, i_o].
...
def train(
    ...
    print(
        f"Fixed L={L_TRUE * 1e6:.0f} uH  C={C_TRUE * 1e6:.0f} uF  "
        f"Init R_L={INIT_R_L:.2f} Ohm  Init R_C={INIT_R_C:.2f} Ohm"
    )
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if epoch == 1 or epoch % 100 == 0:
            ...
            print(
                ...
                f"{p['R_L']:8.5f}  {p['R_C']:8.5f}  {p['delta_G_norm']:8.4f}  "
                f"{p['H_NN_norm']:8.4f}"
            )
    ...
    print(
        f"Learned    : R_L={p['R_L']:.5f} Ohm  "
        f"R_C={p['R_C']:.5f} Ohm  "
        f"delta_G_norm={p['delta_G_norm']:.4f}  "
        f"H_NN_norm={p['H_NN_norm']:.4f}"
    )
    ...
    ckpt = {
        ...
        "dt_train": DT_TRAIN,
        "multistep_n": N_STEPS,
        "fixed_params": {"L": L_TRUE, "C": C_TRUE},
        "learned_params": p,
        "init_params": {"R_L": INIT_R_L, "R_C": INIT_R_C},
    }
    ...
    ax.set_title("SimulationBuckPHNN Training Loss")
...
def evaluate(
    ...
    print("\n" + "=" * 66)
    print("(a) Learned parameters")
    print("=" * 66)
    p = model.get_params()
    print(f"  Fixed L          : {p['L'] * 1e6:.1f} uH")
    print(f"  Fixed C          : {p['C'] * 1e6:.1f} uF")
    print(f"  Init R_L         : {INIT_R_L:.2f} Ohm")
    print(f"  Learned R_L      : {p['R_L']:.5f} Ohm")
    print(f"  Init R_C         : {INIT_R_C:.2f} Ohm")
    print(f"  Learned R_C      : {p['R_C']:.5f} Ohm")
    print(f"  delta_G_norm     : {p['delta_G_norm']:.4f}")
    print(f"  H_NN_norm        : {p['H_NN_norm']:.4f}")
    ...
    fig.suptitle("SimulationBuckPHNN - Predicted vs PLECS", fontsize=11)
    fig.tight_layout()
    ...
    checks = [
        (mean_rmse_iL < RMSE_I_TOL, f"Mean RMSE i_L={mean_rmse_iL:.4e} >= {RMSE_I_TOL}"),
        (mean_rmse_vC < RMSE_V_TOL, f"Mean RMSE V_C={mean_rmse_vC:.4e} >= {RMSE_V_TOL}"),
    ]
